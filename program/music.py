...
@Client.on_message(command(["/mplay","شغيل","شغيل اغنيه","/play","بودا","شغل","تشغيل","ش","صقر", f"/mplay@{BOT_USERNAME}"]) & other_filters)
async def play(c: Client, m: Message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
              ...
              [InlineKeyboardButton("𝙎𝙊𝙐𝙍𝘾𝙀 𝑩𝑶𝑫𝑨 𖡛“.", url=f"https://t.me/Gaamd_23"),
                ],
    ...
    if replied:
        ...
        if len(m.command) < 2:
            ...
            search = ytsearch(query)
            if search == 0:
                await suhu.edit("❌ **لم يتم العثور على نتائج ي لطخ😂**")
            else:
                songname = search[0]
                url = search[1]
